...
# Date:    Sys.Date()
# Purpose: Compute IPCC AR6 regional averages of scPDSI from AWC 100
...
PDSI_recon_01 <- "/Users/phelpap/Documents/lastmilldroughts_data/scPDSI_AWC100/ModE-Sim_m001_PDSI_1420-2009.nc"
...
path  <- "/Users/phelpap/Documents/lastmilldroughts_data/scPDSI_AWC100"
...
for (region in regions) {
  ...
  regional_results_AWC100[[region]]   <- result_array_region
}

cat("\nAll regions processed.\n")

# ── 5. Save output ────────────────────────────────────────────────────────────
out_path <- here::here("data", "regionalResults_scPDSI_1420_2009_AWC100_test.RData")
saveRDS(regional_results_AWC100, file = out_path)
cat(sprintf("Results saved to: %s\n", out_path))

# ── 6. Quick diagnostic plot for GIC / m001 ───────────────────────────────────
if ("MED" %in% names(regional_results_AWC100) &&
    "m001" %in% colnames(regional_results_AWC100[["GIC"]])) {

  pdsi_ts   <- regional_results_AWC100[["GIC"]][, "m001"]
  ...
  p <- ggplot(df_plot, aes(x = time, y = pdsi)) +
    ...
    labs(title = "GIC Regional PDSI — m001 (AWC 100)",
         x = "Year", y = "PDSI") +
    theme_classic()

  plot_path <- here::here("data", "MED_m001_PDSI_timeseries_AWC100.png")
