...
plot_region_PCWD_ts <- function(region_code) {
  ...
  p_ts <- p_ts +
    ...
    geom_hline(data = ensemble_max, aes(yintercept = max_PCWD, color= factor("Ens_maximum")),linetype="dotted", linewidth=0.3)+
...
ts_MED <- plot_region_PCWD_ts("NEU")
ts_MED
```
...
plot_region_PCWD_density <- function(region_code) {
  ...
  # # y-limits shared with time series if needed
  # y_limits     <- range(c(min(dat$MODE_long$PCWD), max(dat$ERA5_df$PCWD)), na.rm=TRUE)
  # common_breaks <- pretty(y_limits, n=6)
...
den_MED <- plot_region_PCWD_density("NEU")
den_MED
```
```{r}
