...
for (region in names(corrected_full)) {
  ...
  region_df <- data.frame(
    time = time_info[15:575],  # adjust to match rolling windows
    RP = rp_window,
    region = region
  )
...
# Combine all regions
combined_results_30yr_pooled <- do.call(rbind, EVA_regional_ensemble)
# Save to file
write.csv(combined_results_30yr_pooled, file = here::here("data/30yr_pooled_return_periods_regions_ERA_members_test.csv"), row.names = FALSE) #ModESim all ensemble members
...
for (region in names(corrected_full)) {
  ...
  time_info <- seq(from = 1420, to = 2009)
  region_df <- data.frame(
    time     = time_info[15:575],  # 561 windows → midpoints of 30yr windows
    RP       = rp_window,
    RP_lower = RP_lower,
    RP_upper = RP_upper,
    region   = region
...
combined_results_30yr_pooled <- do.call(rbind, EVA_regional_ensemble)
write.csv(combined_results_30yr_pooled, 
          file = here::here("data/30yr_pooled_return_periods_regions_ERA_members_test.csv"), 
          row.names = FALSE)
...
combined_results_30yr_ERA <- do.call(rbind, EVA_regional)

# Save or process the combined dataframe
# Example: Save to a CSV file
write.csv(combined_results_30yr_ERA, here::here("data/30yr_pooled_return_periods_regions_ERA5Land_test.csv"), row.names = FALSE)


```

```{r}
# Re-run bootstrap for first window and inspect the distribution
window_first <- result_array_region[26:55]
fit_first    <- fevd(window_first, type = "Gumbel", method = "MLE")
loc_hat      <- extract_loc(fit_first)
scale_hat    <- extract_scale(fit_first)

set.seed(42)
R <- 500
boot_rp_first <- numeric(R)

for (b in seq_len(R)) {
  boot_sample <- revd(30, loc = loc_hat, scale = scale_hat, shape = 0, type = "GEV")
  fit_boot    <- tryCatch(fevd(boot_sample, type = "Gumbel", method = "MLE"), error = function(e) NULL)
  if (is.null(fit_boot)) { boot_rp_first[b] <- NA; next }
  boot_rp_first[b] <- calc_return_period(cwd_ref, extract_loc(fit_boot), extract_scale(fit_boot))
}

# Inspect the distribution
summary(boot_rp_first)
quantile(boot_rp_first, c(0.025, 0.25, 0.5, 0.75, 0.975), na.rm = TRUE)
hist(boot_rp_first, breaks = 50, main = "Bootstrap RP distribution - first window")
```
```{r}
### check distribution of baseline and dataset

# Compare the two baselines
baseline_ensemble <- as.vector(corrected_full[[region]][551:580, ])
baseline_ERA5Land <- result_array_region[1:30]  # or whatever your baseline slice is

cat("Ensemble baseline - mean:", mean(baseline_ensemble), "sd:", sd(baseline_ensemble), "\n")
cat("ERA5Land baseline - mean:", mean(baseline_ERA5Land), "sd:", sd(baseline_ERA5Land), "\n")
cat("cwd_ref from ensemble:", cwd_ref, "\n")
cat("cwd_ref from ERA5Land:", return.level(
  fevd(baseline_ERA5Land, type = "Gumbel", method = "MLE"), 
  return.period = use_rp), "\n")
```
...
#     window_data <- result_array_region[window_start:(window_start + nyears - 1)] # Flatten the matrix into a single vector 
#     window_data <- as.vector(window_data)
#     
#     # Fit Gumbel
#     fit_window <- fevd(window_data, type = "Gumbel", method = "MLE")
#     
#     # Compute return period of baseline event
#     rp_window[window_start] <- calc_return_period(cwd_ref, extract_loc(fit_window), extract_scale(fit_window))
#     
#     # Compute 95% CI for magnitude, then convert to return period - parametric bootstrap to estimate confidence intervals
#     ci_res <- ci(fit_window, 
#              type = "return.level", 
#              return.period = use_rp,
#              method = "boot",
#              R = 250,          # tested and seems stable enough without drawing too many samples (compute time okay)
#              alpha = 0.05)
#   
#   RP_lower[window_start] <- calc_return_period(ci_res[1], extract_loc(fit_window), extract_scale(fit_window))
#   RP_upper[window_start] <- calc_return_period(ci_res[3], extract_loc(fit_window), extract_scale(fit_window))
#   }
#   
#   # Time vector (match rolling windows)
#   time_info <- seq(from = 1970, to = 2024)
...
# # Save or process the combined dataframe
# # Example: Save to a CSV file
# write.csv(combined_results_30yr_ERA, here::here("data/30yr_pooled_return_periods_regions_ERA5Land_ci250.csv"), row.names = FALSE)
...
med_data <- combined_results_30yr_ERA %>% 
  filter(region == "MED")
...
# Filter for region MED
med_data <- combined_results_30yr_pooled %>% 
  filter(region == "MED")
...
ggplot(med_data, aes(x = time, y = RP)) +
  ...
  geom_ribbon(aes(ymin = RP_lower, ymax = RP_upper),           # uncertainty band
              fill = "lightblue", alpha = 0.3) +
  labs(
    title = "Rolling 30-Year Return Period for MED - ModE-Sim",
    x = "Year",
    y = "Return Period (years)"
  ) +
  theme_minimal() +
...
```{r}
## check stats
# For first and last window, compare where cwd_ref sits relative to fitted params
window_first <- result_array_region[1:30]
window_last  <- result_array_region[26:55]

fit_first <- fevd(window_first, type = "Gumbel", method = "MLE")
fit_last  <- fevd(window_last,  type = "Gumbel", method = "MLE")

cat("First window - loc:", extract_loc(fit_first), "scale:", extract_scale(fit_first), "\n")
cat("Last window  - loc:", extract_loc(fit_last),  "scale:", extract_scale(fit_last),  "\n")
cat("cwd_ref:", cwd_ref, "\n")

# Standardised distance of cwd_ref from location
cat("First window z-score:", (cwd_ref - extract_loc(fit_first)) / extract_scale(fit_first), "\n")
cat("Last window  z-score:", (cwd_ref - extract_loc(fit_last))  / extract_scale(fit_last),  "\n")
```
...
combined_df <- read.csv(here::here("data/30yr_pooled_return_periods_regions_ERA_members_test.csv")) #ModESim all ensemble members
combined_era <- read.csv(here::here("data/30yr_pooled_return_periods_regions_ERA5Land_test.csv"))
...
#plot all ensemble members and ensemble mean: 
combined_df <- read.csv(here::here("data/30yr_pooled_return_periods_regions_ERA_members_test.csv"))
combined_era <- read.csv(here::here("data/30yr_pooled_return_periods_regions_ERA5Land_test.csv"))
...
Preparatory analysis for Figure 2: Scatter plot comparing 30 yr drought magnitudes and exceedance maps
...
gg6 <- tmp |> 
  ggplot(aes(x = pcwd_x30_ref0, y = pcwd_x30_ref1, color = color_cat)) +
...
```{r}
# print(row1)
# ggsave(here("figures/test.pdf"), plot = row1, width = 14, height = 7)
```
