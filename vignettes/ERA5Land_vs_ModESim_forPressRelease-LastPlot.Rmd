...
get_pcwd_y_limits <- function(region_code, pcwd_values) {
  upper_limit <- if (region_code == "CHE") {
    150
  } else {
    max(200, ceiling(max(pcwd_values, na.rm = TRUE) / 50) * 50)
  }
...
get_2026_reference_data <- function(region_code, pcwd_2026) {
  if (region_code %in% c("WCE", "CHE")) {
    tibble(pcwd = pcwd_2026)
  } else {
    tibble(pcwd = numeric())
  }
}
...
plot_region_pcwd_ts <- function(region_code) {
  ...
  y_limits <- get_pcwd_y_limits(
    region_code,
    c(data$mode_long$PCWD, data$era5_data$PCWD)
  )
  common_breaks <- get_pcwd_y_breaks(region_code, y_limits)
  record_pcwd <- max(data$mode_long$PCWD, na.rm = TRUE)
  reference_2026 <- get_2026_reference_data(
    region_code,
    data$pcwd_2026
  )
...
pcwd_figure_wna <- build_region_pcwd_figure("WNA")
save_pcwd_figure(
  pcwd_figure_wna,
  "PCWD_WNA.pdf",
  height = 3
...
percentile_table <- percentiles_2026 |>
  transmute(
    Region = recode(region, WCE = "West Central Europe", CHE = "Switzerland", WNA = "Western North America"),
